...
Adding a feature = implement interface + register. No core modifications.

**Solid Frontend — Component registry:**
```typescript
const rendererRegistry = new Map<string, Component<EventProps>>();
rendererRegistry.set('tool_call', ToolCallCard);
rendererRegistry.set('custom:deploy', DeployStatusCard);
```
