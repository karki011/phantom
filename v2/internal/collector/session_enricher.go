...
type SessionEnricher struct {
	...
	extractor *extractor.MemoryExtractor // optional — extracts + embeds session memories
	streamSvc *stream.Service            // optional — provides events for extraction
}
...
// SetExtractor injects a MemoryExtractor for session memory extraction.
// Nil-safe: passing nil disables extraction.
func (se *SessionEnricher) SetExtractor(ex *extractor.MemoryExtractor) {
	se.extractor = ex
}

// SetStreamService injects the stream service for loading session events.
// Required for memory extraction — without it, extraction is skipped.
func (se *SessionEnricher) SetStreamService(svc *stream.Service) {
	se.streamSvc = svc
}
...
func (se *SessionEnricher) EnrichSession(ctx context.Context, sessionID string) {
	...
	// 17. Memory extraction — extract signals from session events and embed them
	// into the VectorStore for semantic retrieval. Optional: skipped when extractor
	// or stream service is nil.
	if se.extractor != nil && se.streamSvc != nil {
		events, evErr := se.streamSvc.GetEvents(ctx, sessionID, 0, 10000)
		if evErr != nil {
			slog.Warn("session_enricher: load events for extraction", "session_id", sessionID, "err", evErr)
		} else if len(events) > 0 {
			result := se.extractor.Extract(sessionID, events)
			if storeErr := se.extractor.Store(ctx, result); storeErr != nil {
				slog.Warn("session_enricher: memory extraction store failed", "session_id", sessionID, "err", storeErr)
			}
		}
	}
}
